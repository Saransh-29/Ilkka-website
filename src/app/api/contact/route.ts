...
// Input validation
function validateContactFormData(data: unknown): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (
    formData.phone &&
    (typeof formData.phone !== 'string' || !validatePhone(formData.phone))
  ) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const bothSuccessful =
      results.notificationResult.success && results.autoReplyResult.success;
    ...
    if (bothSuccessful) {
      ...
      if (
        ...
        results.autoReplyResult.success
      ) {
        return NextResponse.json({
          success: true,
